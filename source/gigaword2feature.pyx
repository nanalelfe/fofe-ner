...
def KBP(filename, iflytek=None, language='eng'):
    ...
    if iflytek is not None:
        generator = chain(generator, imap( lambda x: x[:4], LoadED( iflytek, language=language ) ))
    ...
    def __init__( self, sentence, numericizer, 
        ...
        if language != 'cmn':
            for w in sentence:
                ...
                # logger.info(w)
                self.sentence.push_back( u''.join( c if ord(c) < 128 else str(ord(c)) for c in list(w) ) )
...
class batch_constructor:
    ...
    def mini_batch( self, int n_batch_size, 
        ...
        write_file = codecs.open("write_file.txt", 'w+', 'utf8' )
